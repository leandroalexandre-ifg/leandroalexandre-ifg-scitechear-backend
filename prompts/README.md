...
| `meeting_summary_v1.txt` | PromptSumarizacaoV1.txt | **Ativo (default).** Sumarização (artefato interno). |
| `meeting_summary_v2.txt` | meeting_summary_v1.txt (Movimento 2) | **Em avaliação, não é o default.** Selecionado por `MEETING_SUMMARY_PROMPT_VERSION=v2`. É o v1 com **duas edições e nada mais** (1 linha alterada, 1 adicionada): (1) o campo `Resumo` ganha descrição — era o único campo do schema sem nenhuma, e é justamente onde a invenção pousava, já que todas as regras anti-invenção (11, 16, 17) miram o `Trecho` e a classificação; (2) regra de validação 20, proibindo deduzir/converter/completar dado factual, com o modo de falha nomeado: converter referência temporal relativa em data absoluta. Origem: o v1 escreveu "prazo final (30 de outubro)" citando um trecho que diz "antes desta data", sobre uma transcrição onde "outubro" não aparece nenhuma vez. O lastro exigido é a **transcrição inteira**, não o trecho do próprio elemento — ancorar no trecho reprovaria resolução anafórica legítima (11 resumos de `50098d37` resolvem "ele" para "Rufas", que aparece 12 vezes na transcrição). Critérios de extração, schema, categorias, IDs e formato de saída permanecem idênticos. |
