...
def test_save_and_load_profile_roundtrip(tmp_path):
    ...
    loaded_embedding = repo.load_embedding("p1")
    assert torch.allclose(loaded_embedding, embedding)


def test_load_profile_inexistente_retorna_none(tmp_path):
    repo = VoiceRepository(tmp_path)
    assert repo.load_profile("nao-existe") is None
    assert repo.load_embedding("nao-existe") is None


def test_save_sample_e_list_sample_paths(tmp_path):
    repo = VoiceRepository(tmp_path)
    repo.save_sample("p1", "amostra1.wav", b"conteudo-1")
    repo.save_sample("p1", "amostra2.wav", b"conteudo-2")

    paths = repo.list_sample_paths("p1")
    assert len(paths) == 2
...
def test_list_sample_paths_sem_amostras_retorna_vazio(tmp_path):
    repo = VoiceRepository(tmp_path)
    assert repo.list_sample_paths("ninguem") == []


def test_delete_profile_remove_tudo(tmp_path):
    repo = VoiceRepository(tmp_path)
    repo.save_sample("p1", "amostra.wav", b"conteudo")
    repo.save_profile(
        participant_id="p1", embedding=torch.rand(192), model_version="v1", sample_count=1
    )

    assert repo.delete_profile("p1") is True
    assert repo.load_profile("p1") is None
    assert repo.list_sample_paths("p1") == []


def test_delete_profile_inexistente_retorna_false(tmp_path):
    repo = VoiceRepository(tmp_path)
    assert repo.delete_profile("nao-existe") is False
